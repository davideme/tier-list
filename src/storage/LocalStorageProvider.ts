  ...
  private async saveAllData(data: LocalStorageData): Promise<void> {
    try {
      ...
      if (size > this.maxSize) {
        throw new Error(
          `Data size (${Math.round(size / 1024)}KB) exceeds maximum allowed size (${Math.round(this.maxSize / 1024)}KB)`
        );
      }
      ...
      if (error instanceof Error && error.name === 'QuotaExceededError') {
        throw new Error(
          'Storage quota exceeded. Please delete some tier lists or clear browser data.'
        );
      }
      throw error;
    }
